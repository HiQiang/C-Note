...
void SpeechManager::speehContest()
{
	...
	for (vector<int>::iterator it = v_Src.begin(); it != v_Src.end(); it++)
	{
		...
		//cout << "编号：" << *it << " 选手：" << m_Speaker[*it].m_Name << " 平均分：" << m_Speaker[*it].m_Score[this->m_Index-1] << endl;
		//cout << endl;
...
void SpeechManager::showScore()
{
	...
	{
		cout << "选手编号：" << *it << " 姓名：" << this->m_Speaker[*it].m_Name << " 得分：" << m_Speaker[*it].m_Score[this->m_Index - 1] << endl;
	}
	system("pause");
	system("cls");
	this->show_Menu();
...
void SpeechManager::loadRecord()
{
	...
	//文件不为空
	this->fileIsEmpty = false;
	ifs.putback(ch);//将上面读取的单个字符 放回文件
	int index = 0;
	string data;
	...
	{
		//cout << data << endl;
		vector<string>  v;//存放6个 string 的字符串
		int pos = -1;//查到 " , " 位置的变量
		int start = 0;
